...
static void qt_cleanlooks_draw_mdibutton(QPainter *painter, const QStyleOptionTitleBar *option, const QRect &tmp, bool hover, bool sunken)
{
    ...
    dark.setHsv(option->palette.button().color().hue(),
                qMin(255, static_cast<int>(option->palette.button().color().saturation()*1.9)),
                        ...
                        QPainter *painter, const QWidget *widget) const
{
    ...
    case PE_IndicatorItemViewItemCheck:
        {
            ...
            button2.QStyleOption::operator=(*option);
            button2.state &= ~State_MouseOver;
            proxy()->drawPrimitive(PE_IndicatorCheckBox, &button2, painter, widget);
        }
        return;
    ...
    case PE_IndicatorToolBarSeparator:
        {
            ...
            if (option->state & State_Horizontal) {
                const int offset = rect2.width()/2;
                painter->setPen(QPen(option->palette.window().color().darker(110)));
                painter->drawLine(rect2.bottomLeft().x() + offset,
                            rect2.bottomLeft().y() - margin,
                            rect2.topLeft().x() + offset,
                            rect2.topLeft().y() + margin);
                painter->setPen(QPen(option->palette.window().color().lighter(110)));
                painter->drawLine(rect2.bottomLeft().x() + offset + 1,
                            rect2.bottomLeft().y() - margin,
                            rect2.topLeft().x() + offset + 1,
                            rect2.topLeft().y() + margin);
            } else { //Draw vertical separator
                const int offset = rect2.height()/2;
                painter->setPen(QPen(option->palette.window().color().darker(110)));
                painter->drawLine(rect2.topLeft().x() + margin ,
                            rect2.topLeft().y() + offset,
                            rect2.topRight().x() - margin,
                            rect2.topRight().y() + offset);
                painter->setPen(QPen(option->palette.window().color().lighter(110)));
                painter->drawLine(rect2.topLeft().x() + margin ,
                            rect2.topLeft().y() + offset + 1,
                            rect2.topRight().x() - margin,
                            rect2.topRight().y() + offset + 1);
            }
        }
        break;
    case PE_Frame:
    ...
    case PE_FrameDockWidget:
        ...
        painter->save();
        {
            ...
            painter->drawRect(rect2.adjusted(0, 0, -1, -1));
            painter->setPen(QPen(option->palette.light(), 0));
            painter->drawLine(QPoint(rect2.left() + 1, rect2.top() + 1), QPoint(rect2.left() + 1, rect2.bottom() - 1));
            painter->setPen(QPen(option->palette.window().color().darker(120), 0));
            painter->drawLine(QPoint(rect2.left() + 1, rect2.bottom() - 1), QPoint(rect2.right() - 2, rect2.bottom() - 1));
            painter->drawLine(QPoint(rect2.right() - 1, rect2.top() + 1), QPoint(rect2.right() - 1, rect2.bottom() - 1));
    ...
    case PE_FrameWindow:
        painter->save();
        {
            ...
            painter->drawRect(rect2.adjusted(0, 0, -1, -1));
            painter->setPen(QPen(option->palette.light(), 0));
            painter->drawLine(QPoint(rect2.left() + 1, rect2.top() + 1),
                              QPoint(rect2.left() + 1, rect2.bottom() - 1));
            painter->setPen(QPen(option->palette.window().color().darker(120), 0));
            painter->drawLine(QPoint(rect2.left() + 1, rect2.bottom() - 1),
                              QPoint(rect2.right() - 2, rect2.bottom() - 1));
            painter->drawLine(QPoint(rect2.right() - 1, rect2.top() + 1),
                              QPoint(rect2.right() - 1, rect2.bottom() - 1));
        }
        painter->restore();
        break;
    ...
    case PE_FrameDefaultButton:
        ...
        if (const QStyleOptionFocusRect *focusFrame = qstyleoption_cast<const QStyleOptionFocusRect *>(option)) {
            if (!(focusFrame->state & State_KeyboardFocusChange))
                return;
            QRect rect2 = focusFrame->rect;
            painter->save();
            painter->setBackgroundMode(Qt::TransparentMode);
            painter->setBrush(QBrush(dark.darker(120), Qt::Dense4Pattern));
            painter->setBrushOrigin(rect2.topLeft());
            painter->setPen(Qt::NoPen);
            const QRect rects[4] = {
                QRect(rect2.left(), rect2.top(), rect2.width(), 1),    // Top
                QRect(rect2.left(), rect2.bottom(), rect2.width(), 1), // Bottom
                QRect(rect2.left(), rect2.top(), 1, rect2.height()),   // Left
                QRect(rect2.right(), rect2.top(), 1, rect2.height())   // Right
            };
    ...
    case PE_PanelButtonCommand:
        {
            ...
            if (const QStyleOptionButton *button2 = qstyleoption_cast<const QStyleOptionButton*>(option)) {
                isDefault = (button2->features & QStyleOptionButton::DefaultButton) && (button2->state & State_Enabled);
                isFlat = (button2->features & QStyleOptionButton::Flat);
            }
            ...
            gradientStopColor.setHsv(buttonColor.hue(),
                                     ...
                                     qMin(255, static_cast<int>(buttonColor.value()*0.96)));
                                   ...
                                   const QWidget *widget) const
{
    ...
    QColor darkOutline;
    darkOutline.setHsv(button.hue(),
                qMin(255, static_cast<int>(button.saturation()*2.0)),
                qMin(255, static_cast<int>(button.value()*0.6)));
    QRect rect = option->rect;
    ...
    case CE_HeaderSection:
        ...
        if (const QStyleOptionHeader *header = qstyleoption_cast<const QStyleOptionHeader *>(option)) {
            ...
            gradientStopColor.setHsv(gradientStartColor.hue(),
                                     qMin(255, static_cast<int>(gradientStartColor.saturation()*2)),
                                     qMin(255, static_cast<int>(gradientStartColor.value()*0.96)));
...
bool QCleanlooksStyle::event(QEvent *event)
{
    ...
    case QEvent::Timer: {
        ...
        if (timerEvent->timerId() == animateTimer) {
            ...
            for (QProgressBar *bar : std::as_const(animatedProgressBars))
                bar->update();
        }
                                      ...
                                      const QWidget *widget) const
{
    ...
    case SP_TitleBarNormalButton:
        return QPixmap(static_cast<const char * const *>(dock_widget_restore_xpm));
    case SP_TitleBarMinButton:
        return QPixmap(static_cast<const char * const *>(workspace_minimize));
    case SP_TitleBarCloseButton:
    case SP_DockWidgetCloseButton:
        return QPixmap(static_cast<const char * const *>(dock_widget_close_xpm));
